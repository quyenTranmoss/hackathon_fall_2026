...
export default function SwellProfile({ profileData, profileState, currentUserProfileState, goals, onGoalsChange, onAssessmentChange, onAssessmentComplete, onClose }) {
  ...
  const screen = hasDashboard ? {
    ...
    Traits: <SwellTraits profile={dashboardData.profile} dashboardData={dashboardData} isOwnProfile={isOwnProfile} />,
  ...
  return <div className="modal-backdrop">
    <section className="profile-modal" aria-label={`${profile.name} profile`}>
      ...
      {topTab === 'Swell Profile' && needsAssessment ? <SwellAssessment assessment={profileState.assessment} onChange={onAssessmentChange} onComplete={onAssessmentComplete} /> : topTab === 'Swell Profile' && hasDashboard ? <>
        <SwellTabs active={swellTab} isOwnProfile={isOwnProfile} onChange={setSwellTab} />
        <div className="swell-content">{screen[swellTab]}</div>
      </> : topTab === 'Swell Profile' ? <div className="placeholder-panel"><p>Profile loading</p><span>Your Swell profile information is being prepared.</span></div> : <div className="placeholder-panel"><p>{topTab}</p><span>This Teams profile section is shown only as surrounding context for the Swell prototype.</span></div>}
    </section>
  </div>;
}
